...
import { GET } from "@/app/api/dashboard/team-utilization/route";
...
describe("GET /api/dashboard/team-utilization", () => {
  beforeEach(() => {
  ...
  it("returns 401 unauthenticated", async () => {
    const req = new NextRequest("http://localhost/api/dashboard/team-utilization");
    expect((await GET(req)).status).toBe(401);
  });

  it("returns array for authenticated user", async () => {
    mockAuth.mockResolvedValue(memberSession);
    const req = new NextRequest("http://localhost/api/dashboard/team-utilization?from=2025-06-01&to=2025-08-31");
